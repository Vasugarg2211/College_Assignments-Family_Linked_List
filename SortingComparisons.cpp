#include <iostream>              //Vasuman 
using namespace std;            //SID:21105083
...
void bubble_sort(int arr[],int n) // bubble sort algo
{ 
    int i=0,j=1,t=n-1;
    int swaps=0;
    int compar=0;
    ...
    cout<<"number of comparisons:"<<compar<<endl;
    cout<<"[implace algo used]"<<endl;
}


//function that returns index of minimum value in an array
int min_element(int arr[],int si,int ei){
    int m=INT_MAX;
    int j=si;           //j=index holding minimum value
    for(int i=si;i<=ei;i++){
...
void selection_sort(int arr[],int n){
    ...
    while(si<ei)
    {
        int mi=min_element(arr,si,ei);  //mi=index holding minimum value
        compar=compar+(ei-si+1);
        if(si!=mi){
            swap(arr[si],arr[mi]);
            swaps++;
        }
